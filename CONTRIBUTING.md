...
`web/registry_curation.json` controls how an extension appears on the site. Add an entry keyed
by your slug:

```json
"my-extension-name": {
  "tags": ["ambient", "newtab"],
  "art_image": "assets/my_preview.jpg"
}
```

- `tags` must come from `ambient`, `focus`, `newtab`, `tech` (these are the gallery filters).
- `art_image` must exist under `web/`. If you skip it, a file named `assets/<slug>.jpg|png|svg`
  is picked up automatically, otherwise a default is used.
- `featured: true` pins an extension to the large hero card. Only one may have it.
